...
<div class="panel panel-default org-panel-2 organization-panel personal-details-panel">
    ...
    <label><b>Country : </b><g:if test="${project.beneficiary.city}">${project.beneficiary.city}</g:if>, ${project.beneficiary.country}</label><br>
    ...
    <g:if test="${project.rewards.size() > 1}">
        <label><b>Perks : </b>${project.rewards.size()}</label>
    </g:if>
    <g:else>
        <label><b>Perks : </b> No Rewards</label>
    </g:else>
    <label><b>,Teams : </b>${project.teams.size()}</label><br>
